...
import { binPrices } from "@/app/lib/utils";
...
function PriceHistogramTooltip(props: CustomBarChartTooltipProps) {
      ...
      <div>
        ...
        <div>
          ...
          <span className="text-zinc-900 font-semibold">
            {numberToGold(payload[0]?.payload?.binStart)}-{numberToGold(payload[0]?.payload?.binEnd)}
          </span>
        </div>
      </div>
    </div>
...
function PriceDistributionHistogramChart({ plotData }: { plotData: QuantityAtPrice[] }) {
  if (!plotData || plotData.length === 0) {
    return <div className="text-zinc-400 my-24">Current Listing data is not available</div>;
  }
  const binned = binPrices(plotData, 1);
          ...
          <BarChart data={binned}>
            ...
            <Tooltip
              ...
              content={(tooltipProps) => (
                <PriceHistogramTooltip {...tooltipProps} yLabel={"Quantity"} xLabel={"Price"} />
              )}
            />
            <Brush dataKey="price" height={30} stroke="#a5f3fc" />
...
export default PriceDistributionHistogramChart;
